  ...
  const handleMine = useCallback(async () => {
    try {
      ...
      if (data.success) {
        setUser((prev) => ({
          ...prev,
          points_balance: prev.points_balance + (data.points || MINING_CONSTANTS.POINTS_PER_MINE),
          ...
          longest_streak: Math.max(prev.longest_streak || 0, data.streak || (prev.current_streak || 0) + 1),
        }))
            ...
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  You received <span className="text-primary font-bold">{MINING_CONSTANTS.WELCOME_BONUS} VP</span> for
                  joining!
                </p>
              </div>
            </CardContent>
            ...
            <CardContent className="p-6 relative z-10">
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex-1 flex flex-col md:flex-row items-center gap-6">
                  ...
                  <PointsDisplay points={user.points_balance} animate />
                </div>
                <MiningButton
                    ...
                    <div className="text-right">
                      ...
                      <div className="w-24 h-1.5 bg-muted rounded-full mt-1 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${((user.current_streak || 0) / nextMilestone.days) * 100}%` }}
                        />
                      </div>
                    </div>
                  )}
              ...
              {currentMultiplier > 1 && (
                <div className="flex justify-between items-center p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
                  ...
                  <Badge className="bg-orange-500/20 text-orange-400 font-bold">
                    {Math.floor(MINING_CONSTANTS.POINTS_PER_MINE * currentMultiplier)} VP ({currentMultiplier}x)
                  </Badge>
                </div>
              )}
              <div className="flex justify-between items-center p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                <span className="text-muted-foreground">Cooldown</span>
                <span className="font-bold text-foreground">{MINING_CONSTANTS.MINING_COOLDOWN_HOURS} hours</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
              ...
              {Object.entries(MINING_CONSTANTS.STREAK_BONUS_MULTIPLIERS).map(([days, mult]) => {
                ...
                const isNext =
                  ...
                  Object.keys(MINING_CONSTANTS.STREAK_BONUS_MULTIPLIERS)
                    ...
                    .every((d) => (user.current_streak || 0) >= d || d > (user.current_streak || 0))
                ...
                return (
                  ...
                  >
                    ...
                    <Badge
                      ...
                      className={cn(
                        "text-xs",
                        isAchieved ? "border-orange-500/50 text-orange-400" : "border-muted-foreground/30",
                      )}
                    >
                      {mult}x bonus
                    </Badge>
          ...
          <Card className="bg-primary/5 backdrop-blur-sm border-primary/20 hover:border-primary/40 transition-colors">
            ...
            <CardContent className="space-y-3 text-sm">
              {[
                ...
                { text: "Share your referral link for bonus points", icon: Users },
