...
export function AdminRanksContent({ rankConfigs }: { rankConfigs: RankConfig[] }) {
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch("/api/admin/ranks/save", {
        method: "POST",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch("/api/admin/ranks/delete", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        ...
        <CardContent>
          <div className="space-y-3">
            {rankConfigs.map((config) => {
              ...
              return (
                <div key={config.id} className={cn("p-4 rounded-lg border", colors.bg, colors.border)}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {config.points_threshold.toLocaleString()} VP + {config.referrals_required} referrals
                        </p>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, points_threshold: Number.parseInt(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, referrals_required: Number.parseInt(e.target.value) })}
                />
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, referral_bonus_multiplier: Number.parseFloat(e.target.value) })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, daily_mining_reward: Number.parseInt(e.target.value) })}
                />
              </div>
            </div>
            <div className="flex gap-2">
