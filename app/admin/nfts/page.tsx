...
export default async function AdminNFTsPage() {
  ...
  // Get NFT sales stats
  const {data: nftSales} = await supabase
    .from("nft_transactions")
    .select("*")
    .eq("transaction_type", "purchase");

  // Group sales by NFT
  const salesByNft = (nftSales || []).reduce((acc, sale) => {
        ...
        {/* <Card className="bg-card border-border">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">
              All available NFTs and their stats
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {nftCatalog?.map((nft: NFTCatalog) => {
                ...
                return (
                  <div
                    key={nft.id}
                    className={cn("p-4 rounded-lg", tierColors.bg, tierColors.border, "border")}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <Badge className={cn("text-xs", tierColors.bg, tierColors.text)}>
                            {nft.tier}
                          </Badge>
                        </div>
                      </div>
              ...
              {recentTransactions?.map(
                (
                  tx: NFTTransaction & {
                    ...
                    nft?: {name: string; tier: string};
                  }
                ) => {
                  const tierColors = tx.nft
                    ? NFT_TIER_COLORS[tx.nft.tier as NFTTier]
                    : NFT_TIER_COLORS.basic;
                  ...
                  return (
                    <div
                      ...
                      <div className="text-right">
                        {tx.vp_amount && (
                          <p className="text-sm font-medium text-primary">{tx.vp_amount} VP</p>
                        )}
