...
import { Users, Pickaxe, Coins, AlertTriangle, TrendingUp, UserPlus, Store, Rocket } from "lucide-react"
...
export default async function AdminDashboard() {
  ...
  const rankCounts = (rankDistribution || []).reduce(
    (acc, r) => {
  ...
  // Get NFT stats
  const { count: totalNFTsSold } = await supabase.from("user_nfts").select("*", { count: "exact", head: true })
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-foreground mt-1">{totalMiningSessions || 0}</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-foreground mt-1">{currentPhase?.phase_number || 1}</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{totalNFTsEquipped || 0} equipped</p>
              </div>
            ...
            <div className="flex items-center justify-between gap-2">
              {(["rookie", "silver", "gold", "diamond", "citizen"] as RankName[]).map((rank) => (
                <div key={rank} className="flex flex-col items-center">
                  ...
                  <span className="text-sm font-bold text-foreground mt-1">{rankCounts[rank] || 0}</span>
                </div>
              ))}
            </div>
          </CardContent>
