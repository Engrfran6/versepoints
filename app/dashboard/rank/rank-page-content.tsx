...
export function RankPageContent({ currentRank, totalPoints, rankConfigs, rankHistory }: RankPageContentProps) {
  return (
                ...
                <div key={log.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{new Date(log.awarded_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                  {log.mining_boost_percentage && (
                    <span className="text-sm text-primary">+{log.mining_boost_percentage}% boost</span>
                  )}
                </div>
              ))}
            </div>
