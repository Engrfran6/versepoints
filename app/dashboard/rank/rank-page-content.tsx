...
import type {RankName, RankConfig, RankRewardsLog, UserRank} from "@/lib/types/phase2";
import {useRankConfigs, useRankHistory, useRankUser} from "@/lib/hooks/useRank";
import {DashboardSkeleton} from "../../../components/skeleton/DashboardSkeleton";
import {RankSkeleton} from "@/components/skeleton/rank-skeleton";
...
export function RankPageContent({userId}: RankPageContentProps) {
  const {data: user, isLoading: userLoading} = useRankUser(userId);
  const {data: rankConfigs, isLoading: rankConfigsLoading} = useRankConfigs();
  const {data: rankHistory, isLoading: rankHistoryLoading} = useRankHistory(userId);

  const isLoading = userLoading || rankConfigsLoading || rankHistoryLoading;

  const currentRank = user?.user_current_rank || "rookie";
  const totalPoints = user?.total_mined || 0;
  const userReferrals = user?.total_referred || 0;
