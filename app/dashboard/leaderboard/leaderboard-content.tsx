...
export function LeaderboardContent({ leaderboard, userRank, currentUserId }: LeaderboardContentProps) {
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-3xl font-bold text-primary">{userRank.points_balance.toLocaleString()} VP</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {leaderboard && leaderboard.length >= 3 && (
        <div className="relative z-10 grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="flex flex-col items-center pt-8">
            ...
            <p className="font-bold text-foreground text-center truncate w-full">{leaderboard[1]?.username}</p>
            <p className="text-sm text-muted-foreground">{leaderboard[1]?.points_balance.toLocaleString()} VP</p>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="font-bold text-foreground text-center truncate w-full text-lg">{leaderboard[0]?.username}</p>
            <p className="text-sm text-primary font-bold">{leaderboard[0]?.points_balance.toLocaleString()} VP</p>
          </div>
          ...
          <div className="flex flex-col items-center pt-12">
            ...
            <p className="font-bold text-foreground text-center truncate w-full">{leaderboard[2]?.username}</p>
            <p className="text-sm text-muted-foreground">{leaderboard[2]?.points_balance.toLocaleString()} VP</p>
          </div>
        </div>
      )}
            ...
            {leaderboard?.map((entry) => (
              <div
                ...
                className={cn(
                  ...
                  entry.id === currentUserId ? "bg-primary/10 border-primary/30" : getRankStyle(entry.rank),
                )}
              >
                <div className="flex items-center gap-4">
                  <div className="w-8 flex justify-center">{getRankIcon(entry.rank)}</div>
                  <div>
                    <p className={cn("font-medium", entry.id === currentUserId ? "text-primary" : "text-foreground")}>
                      {entry.username}
                      {entry.id === currentUserId && (
                        <span className="ml-2 text-xs bg-primary/20 text-primary px-2 py-0.5 rounded-full">You</span>
                      )}
                    </p>
                    <p className="text-xs text-muted-foreground">{entry.mining_count} mining sessions</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-foreground">{entry.points_balance.toLocaleString()}</p>
