  ...
  try {
    ...
    if (userData.last_mining_at) {
      ...
      if (now - lastMining < cooldownMs) {
        ...
        return NextResponse.json({ success: false, error: `Mining available in ${timeLeft} minutes` }, { status: 429 })
      }
    }
    ...
    const { data: recentMiningFromIP } = await supabase
      ...
      .gte("created_at", new Date(Date.now() - RATE_LIMITS.MINING_PER_IP_HOURS * 60 * 60 * 1000).toISOString())
      .neq("user_id", user.id)
      .limit(1)
    ...
    if (recentMiningFromIP && recentMiningFromIP.length > 0) {
      ...
      return NextResponse.json({ success: false, error: "Mining limit reached for this network" }, { status: 429 })
    }
    ...
    if (!existingFingerprint) {
      ...
      if (otherUserFingerprint && otherUserFingerprint.length > 0) {
        ...
        return NextResponse.json({ success: false, error: "Device verification failed" }, { status: 403 })
      }
    ...
    return NextResponse.json({
      success: true,
      points: pointsEarned,
      newBalance: userData.points_balance + pointsEarned,
      streak: newStreak,
      multiplier,
      streakBroken,
