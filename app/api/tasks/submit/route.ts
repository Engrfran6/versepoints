...
export async function POST(request: NextRequest) {
  try {
    ...
    const {taskId, proofUrl, proofData, verificationType} = await request.json();
    ...
    const {error: insertError} = await supabaseAdmin.from("task_submissions").insert({
      user_id: user.id,
      task_id: taskId,
      status: verificationType === "youtube" ? "verified" : "pending",
      proof_url: verificationType === "youtube" ? null : proofUrl || null,
